...
# ✅ 问题8修复：Render免费版自动降低并发
MAX_WORKERS  = 2 if os.getenv('RENDER') else 5
MAX_RETRIES  = 3
...
def call_glm(prompt: str, temperature: float = 0.1) -> str:
    # ✅ 问题9修复：简单缓存相同prompt的结果
    prompt_hash = hash(prompt[:100])
    if prompt_hash in API_CALL_CACHE:
        return API_CALL_CACHE[prompt_hash]
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            # ✅ 问题8修复：缩短超时时间到30秒（Render限制）
            resp = requests.post(url, headers=headers, json=data, timeout=30)
            resp.raise_for_status()
            result = resp.json()['choices'][0]['message']['content'].strip()
            
            # 缓存结果
            API_CALL_CACHE[prompt_hash] = result
            return result
...
def ai_extract_keywords(reviews_text: str, sentiment: str, category: str = "商品") -> dict:
    ...
    # ✅ 问题9修复：限制文本长度避免超时
    reviews_text = reviews_text[:2000]
...
要求：
1. 提取10-15个最有代表性的词语或短语（2-6字为佳）
2. 关键词必须和【{category}】这类商品直接相关
3. 每个词给一个权重分数（1-100）
4. 只返回JSON格式，格式：{{"词语": 权重, "词语": 权重}}"""
    ...
    try:
        ...
        if match:
            parsed = json.loads(match.group())
            return {k: int(v) if isinstance(v, (int, float)) else 50 
