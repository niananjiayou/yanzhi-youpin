...
# ✅ v4.3：中文停用词表（虚词、连接词、无实意词）
CHINESE_STOPWORDS = {
    # 代词
    "的", "了", "是", "我", "你", "他", "她", "它", "们", "我们", "你们", "他们",
    # 介词/虚词
    "在", "有", "很", "和", "或", "不", "没", "都", "也", "就", "还", "只",
    "可以", "这", "那", "这个", "那个", "一个", "一些", "一点", "多", "少",
    # 形容词虚词
    "大", "小", "好", "坏", "真", "假", "对", "错", "等等", "之类",
    # 语气词
    "吗", "呢", "啊", "哦", "呀", "嗯", "嗨", "嘿",
    # 连接词
    "比如", "例如", "因为", "所以", "但是", "然而", "虽然", "尽管",
    # 数字
    "一", "二", "三", "四", "五", "六", "七", "八", "九", "十",
    # 其他虚词
    "吧", "的话", "来说", "而言", "何", "到", "给", "把"
...
def ai_extract_keywords(reviews_text: str, sentiment: str, category: str = "商品") -> dict:
    """
    从评论中提取与产品优缺点直接相关的有意义词汇
    ✅ v4.3：停用词过滤 + AI 智能识别
    """
    if not reviews_text.strip():
        return {}

    prompt = f"""你是电商产品分析专家，请从用户【{sentiment}】评论中，
只提取与产品【优点/缺点】直接相关的有意义词汇。

【严格规则】：
✅ 可以提取：质量好、物流快、包装精美、易上手、续航久、做工细致、性价比高
❌ 不可以提取：的、了、我、你、很、和、或、这个、那个、吗、呢、等虚词

评论内容：
{reviews_text[:3000]}

要求：
1. 只提取名词、形容词、动词短语（2-6字）
2. 必须与【{category}】产品的实际特性直接相关
3. 不含虚词（的、了、我、你、很、和等）
4. 权重计算：词出现频率×10 + 基础分40
5. 列表不超过20个词
6. 只返回JSON格式：{{"词": 权重}}"""

    result = call_glm(prompt, temperature=0.15)  # 降低温度确保准确性
    ...
    try:
        ...
        if match:
            ...
            # 3. 确保值是整数
            filtered = {
                k: int(v) if isinstance(v, (int, float)) else 50
                for k, v in parsed.items()
                if k not in CHINESE_STOPWORDS and len(k) >= 2
            }
            return filtered
    except Exception as e:
        print(f"    ⚠️  关键词解析失败: {e}")
